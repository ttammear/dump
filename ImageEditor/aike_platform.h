...
struct AikePlatform
{
    ...
    void (*resize_window)(AikePlatform *platform, AikeWindow *window, uint32_t width, uint32_t height);
...
inline double aike_timedif_sec(AikeTime from, AikeTime to)
{
    double ret = (to.sec - from.sec) + (((int64_t)to.nsec - (int64_t)from.nsec) / 1000000000.0);
    double fromSec = to.sec - from.sec;
    return ret;
}
